...
> **Reference implementation.** This repo ships a worked example at [`bi_connectors/example_bi/`](bi_connectors/example_bi/) — a complete connector verified end to end by the `bi_connection`/`bi_metadata` tests. It's the best model for structure and idioms.
...
custom-connector-setup/
  ...
  bi_connectors/
    ...
    <your-bi-tool>/                       # Created by you
      ...
      credentials.json                    # Vendor API credentials (gitignored)
      manifest.json                       # connection_type, name, asset_class "bi", terminology, credentials_schema
      requirements.txt                    # Vendor client library deps
      Dockerfile.extra                    # System dependencies (optional)
    example_bi/                           # Reference BI connector (worked example)
