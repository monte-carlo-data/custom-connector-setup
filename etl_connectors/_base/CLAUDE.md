...
- `EtlRunEvent` accepts the same optional nested `group` dict (`source_id` required) to attribute a run to a specific placement (group-instance). Backend mints the internal `group_global_id` from `source_id`; connectors never supply that id. Available since pycarlo 0.12.514.
