...
class MetadataQueryTemplates:
    ...
    def get_tables_query_template(self) -> str:
        return (
            ...
            "NULLIF((SELECT c.reltuples::BIGINT FROM pg_class c JOIN pg_namespace n ON n.oid = c.relnamespace WHERE n.nspname = t.table_schema AND c.relname = t.table_name), -1) AS row_count, "
            "pg_total_relation_size(quote_ident(t.table_schema) || '.' || quote_ident(t.table_name)) AS byte_count, "
            "TO_CHAR(GREATEST(pg_stat.last_autoanalyze, pg_stat.last_analyze) AT TIME ZONE 'UTC', 'YYYY-MM-DD HH24:MI:SS') AS last_update_time, "
            "v.view_definition AS view_query "
            ...
            "{% if table_names is defined and table_names %}AND t.table_name IN ({{ table_names }}){% endif %} "
...
class QueryLanguageTemplates:
    ...
    def utc_literal_template(self) -> str:
        return "{% if timestamp is defined %}TIMESTAMP WITH TIME ZONE '{{ timestamp.strftime('%Y-%m-%d %H:%M:%S') }}+00'{% else %}NOW() AT TIME ZONE 'UTC'{% endif %}"
    ...
    def get_date_diff_func_template(self) -> str:
        return "EXTRACT(EPOCH FROM ({{ date_expr2 }} - {{ date_expr1 }})) / 86400"
    ...
    def get_regexp_count_expression_template(self) -> str:
        return "(SELECT COUNT(*) FROM REGEXP_MATCHES({x}, {{ regexp }}, '{% if case_insensitive %}gi{% else %}g{% endif %}'))"
    ...
    def unpivot_template(self) -> str:
        return (
            ...
            "FROM ({{ from_table }}) src "
            "CROSS JOIN LATERAL ("
            "VALUES {% for col in column_list %}('{{ col }}', src.{{ col }}){% if not loop.last %}, {% endif %}{% endfor %}"
            ") AS unpivoted({{ name_column }}, {{ value_column }})"
        )
...
class FunctionalTestOperations:
    ...
    def insert_rows_template(self) -> str:
        return "INSERT INTO {{ schema }}.{{ table }} (value) SELECT 'row_' || g FROM generate_series(1, {{ num_rows }}) g; ANALYZE {{ schema }}.{{ table }}"
