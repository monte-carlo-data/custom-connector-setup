...
**Placeholder-template methods** (no params in monolith, return `{x}`, `{y}` placeholder strings):

```python
# Pandora:  def get_is_eq_expression_template(self): return "{x} = {y}"
# Monolith: def get_is_eq_expression(self) -> str: return "{x} = {y}"
```

These already use the same `{x}`/`{y}` format-string placeholders as monolith — copy them directly.

**Parameterized methods** (take params in monolith, use Jinja `{{ var }}` in pandora):

```python
# Pandora:  def get_safe_divide_template(self): return "CASE WHEN {{ divisor }} = 0 THEN NULL ELSE {{ dividend }} / {{ divisor }} END"
...
#               return f"CASE WHEN {divisor} = 0 THEN NULL ELSE {dividend} / {divisor} END"
```

Convert `{{ var }}` to `{var}` inside an f-string. The Jinja variable names match the monolith method parameter names.
...
- `get_case_when_func`: Both pandora and monolith use `conditions_and_results: list[tuple[str, str]], else_result: str | None`.
